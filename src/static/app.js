                  ...
                  .map(
                    (email) =>
                      `<li class="participant-item" style="display: flex; align-items: center; justify-content: space-between;">
                        <span>${email}</span>
                        <button class="delete-participant-btn" title="Remove participant" data-activity="${name}" data-email="${email}" style="background: none; border: none; color: #e53935; cursor: pointer; font-size: 18px; margin-left: 8px;">
                          &#128465;
                        </button>
                      </li>`
                  )
              ...
              if (confirm(`Remove ${email} from ${activityName}?`)) {
                try {
                  const res = await fetch(`/unregister`, {
                    ...
                    body: JSON.stringify({ activity: activityName, email })
                  });
                  if (res.ok) {
